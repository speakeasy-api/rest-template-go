//go:generate mockgen -destination=./mocks/users_mock.go -package mocks faceittechtest/internal/users Store,Events
...
const (
	...
	ErrInvalidFilterValue = app.Error("invalid_filter_value: invalid filter value")
	// ErrInvalidFilterMatchType is returned when a filter match type is not found in the supported enum list
	ErrInvalidFilterMatchType = app.Error("invalid_filter_match_type: invalid filter match type")
	// ErrInvalidFilterField is returned when a filter field is not found in the supported enum list
	ErrInvalidFilterField = app.Error("invalid_filter_field: invalid filter field")
)
